...
# BUT - right now - when looking a level ahead, I'm also looking a level behind and take all those options into account for penalties - wrong?
			...
			if(r2[slev+1]>0){
				...
				if(r2[slev-1]>0){
					...
					for(i in 1:n){#trait for previous level
						for(j in 1:n){#trait for this level
							for(k in 1:n){#trait for next level
								payoffs[i,j,k] <- r[slev,j] + penalties[[3]][i,j,k]
							}
						}
					}
					best <- which(payoffs == max(payoffs),arr.ind=T)
					r2[slev] <- ifelse(nrow(best)>1,best[sample(nrow(best),1),2], best[1,2])
				}
			}
		}
		else if(s-slev>=1){
			...
			if(r2[slev+1]>0){
				...
				if(r2[slev-1]>0 & r2[slev-2]>0){
					...
					if(r2[slev-1]>0){
						...
						for(i in 1:n){
							for(j in 1:n){#
								payoffs[i,j] <- r[slev,i] + penalties[[slev+1]][j,i,r2[slev+1]]
							}
						}
						best <- which(payoffs == max(payoffs), arr.ind=T)
						r2[slev] <- ifelse(nrow(best)>1,best[sample(nrow(best),1),1], best[1,1])
					...
					if(r2[slev-1]>0){
						...
						for(i in 1:n){#trait for previous level
							for(j in 1:n){#this is the trait for this level
								for (k in 1:n){#trait for next level
									payoffs[i,j,k] <- r[slev,j] + penalties[[slev+1]][i,j,k]
								}
							}
						}
						best <- which(payoffs == max(payoffs), arr.ind=T)
				...
				if(r3[slev+1]>0){
					...
					if(r3[slev+2]>0){
						...
						if (r3[slev+1]>0){
							...
							for(i in 1:n){#trait for previous level
								for(j in 1:n){
									payoffs[i,j] <- r[slev,j] + penalties[[slev+1]][i,j,r3[slev+1]] + penalties[[slev+2]][j,r3[slev+1],r3[slev+2]]
								}
							}
							best <- which(payoffs == max(payoffs), arr.ind=T)
						...
						for(i in 1:n){#trait for previous level
							for(j in 1:n){#trait for this level
								for(k in 1:n){#trait for 4th level
									payoffs[i,j,k] <- r[slev,j] + penalties[[slev+1]][i,j,r3[slev+1]] + penalties[[slev+2]][j,r3[slev+1],k]
								}
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
					...
					if(r3[slev+2]>0){
						...
						payoffs <- array(0, dim=c(n,n,n))
						for(i in 1:n){#previous level
							for(j in 1:n){#this level
								for(k in 1:n){#next level
									payoffs[i,j,k] <- r[slev,j] + penalties[[slev+1]][i,j,k] + penalties[[slev+2]][j,k,r3[slev+2]]
								}
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
						...
						for(i in 1:n){#prev level
							for(j in 1:n){#this level
								for(k in 1:n){#next level
									for(l in 1:n){#4th level
										payoffs[i,j,k,l] <- r[slev,j] + penalties[[slev+1]][i,j,k] + penalties[[slev+2]][j,k,l]
									}
								}
							}
						}
			...
			if(r3[slev+1]>0){
				...
				if(r3[slev-1]>0 & r3[slev-2]>0){
					...
					if(r3[slev-1]>0){
						...
						for(i in 1:n){
							for(j in 1:n){#
								payoffs[i,j] <- r[slev,i] + penalties[[slev+1]][j,i,r3[slev+1]]
							}
						}
						best <- which(payoffs == max(payoffs), arr.ind=T)
						r3[slev] <- ifelse(nrow(best)>1,best[sample(nrow(best),1),1], best[1,1])

###################################################
## INDIVIDUAL LEARNING STAGE OF SIMULATION

## go through a learned repertoire in random order, and do x-step learner - if the payoff is bigger with the new variant, keep new trait
## x step learner - look at x steps, learn 1
...
# ALSO - if we don't know the previous level, learn from scratch
...
one_learner_update <-function(r,sdev,model_rep){
	sdev <- sdev
	n <- ncol(r)
	s <- nrow(r)
	a <- 1:n
	penalties<- calculate_penalties(s,n,r,a)
	to_learn <- sample(s)
	repertoire <-  model_rep
	for (slev in to_learn){
		if(slev==1){
			#pick best
			best <- which(r[slev,] == max(r[slev,]))
			best_pick <- ifelse(length(best)>1,best[sample(length(best),1)], best[1])
			#compare payoffs
			if(r[slev,best_pick] > r[slev,model_rep[slev]]){
				repertoire[,slev] <- best_pick
			}
		}
		else if(slev==2){
			if(repertoire[slev-1]>0){
				#if we know the previous level
				penalty <- penalties[[slev]][repertoire[slev-1],]
			...
			#compare payoffs
			if(penalties[[2]][repertoire[1],best_pick] > penalties[[2]][repertoire[1],repertoire[2]]){
				repertoire[,slev] <- best_pick
			}
		}
		else{
			...
			if(penalties[[slev]][repertoire[slev-2],repertoire[slev-1],best_pick] > penalties[[slev]][repertoire[slev-2],repertoire[slev-1],repertoire[slev]]){
				repertoire[,slev] <- best_pick
			}			
		}
	}
	tp <- calculate_payoff(s,n,r,a,repertoire)
	ans <- list(repertoire, tp)
	return(ans)
}

##
two_learner_update <- function(r, sdev, model_rep){
	sdev <- sdev
	n <- ncol(r)
	s <- nrow(r)
	a <- 1:n
	...
	for (slev in to_learn){
		...
		if(slev == 1){
			...
			if (r2[slev+1]>0){
				...
				best_pick <- ifelse(nrow(best)>1,best[sample(nrow(best),1),1], best[1,1])
			}
		} 
		...
		if (slev==1){
			...
			if(penalties[[2]][r2[1],best_pick] > penalties[[2]][r2[1],r2[2]]){
				r2[,slev] <- best_pick
			}
		}
		else{
			if(penalties[[slev]][r2[slev-2],r2[slev-1],best_pick] > penalties[[slev]][r2[slev-2],r2[slev-1],r2[slev]]){
				r2[,slev] <- best_pick
			}	
		}
	}
	#calculate payoff
	calculate_payoff(s,n,r,a,repertoire)
	ans <- list(r2, tp)
	return(ans)	
}

##
three_learner_update <- function(r,sdev, to_learn){
    sdev <- sdev
	n <- ncol(r)
	s <- nrow(r)
	a <- 1:n
	...
	for (slev in to_learn){
				...
				if(r3[slev+1]>0){
					...
					if(r3[slev+2]>0){
						...
						if (r3[slev+1]>0){
							payoffs <- matrix(0,ncol=n,nrow=1)
							for(i in 1:n){
								payoffs[i] <- penalties[[2]][r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,r3[slev+1]] + penalties[[slev+2]][i,r3[slev+1],r3[slev+2]]
							}
							best <- which(payoffs == max(payoffs))
							best_pick <- ifelse(length(best)>1,best[sample(length(best),1)], best[1])
						} 
						...
						for(i in 1:n){
							for(j in 1:n){
								payoffs[i,j] <- penalties[[2]][r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,r3[slev+1]] + penalties[[slev+2]][i,r3[slev+1],j]
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
						best_pick <- ifelse(nrow(best)>1,best[sample(nrow(best),1),1], best[1,1])
					...
					if(r3[slev+2]>0){
						...
						for(i in 1:n){
							for(j in 1:n){
								payoffs[i,j] <- penalties[[2]][r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,j] + penalties[[slev+2]][i,j,r3[slev+2]]
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
						best_pick <- ifelse(nrow(best)>1,best[sample(nrow(best),1),1], best[1,1])
						...
						for(i in 1:n){
							for(j in 1:n){
								for(k in 1:n){
									payoffs[i,j,k] <- penalties[[2]][r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,j] + penalties[[slev+2]][i,j,k]
								}
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
				...
				if(r3[slev+1]>0){
					...
					if(r3[slev+2]>0){
						...
						if (r3[slev+1]>0){
							payoffs <- matrix(0,ncol=n,nrow=1)
							for(i in 1:n){
								payoffs[i] <- penalties[[slev]][r3[slev-2],r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,r3[slev+1]] + penalties[[slev+2]][i,r3[slev+1],r3[slev+2]]
							}
							best <- which(payoffs == max(payoffs))
							best_pick <- ifelse(length(best)>1,best[sample(length(best),1)], best[1])
						} 
						...
						for(i in 1:n){
							for(j in 1:n){
								payoffs[i,j] <- penalties[[slev]][r3[slev-2],r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,r3[slev+1]] + penalties[[slev+2]][i,r3[slev+1],j]
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
						best_pick <- ifelse(nrow(best)>1,best[sample(nrow(best),1),1], best[1,1])
					...
					if(r3[slev+2]>0){
						...
						for(i in 1:n){
							for(j in 1:n){
								payoffs[i,j] <-penalties[[slev]][r3[slev-2],r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,j] + penalties[[slev+2]][i,j,r3[slev+2]]
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
						best_pick <- ifelse(nrow(best)>1,best[sample(nrow(best),1),1], best[1,1])
						...
						for(i in 1:n){
							for(j in 1:n){
								for(k in 1:n){
									payoffs[i,j,k] <- penalties[[slev]][r3[slev-2],r3[slev-1],i] + penalties[[slev+1]][r3[slev-1],i,j] + penalties[[slev+2]][i,j,k]
								}
							}
						}
						best <- which(payoffs == max(payoffs),arr.ind=T)
		...
		if (slev==1){
			...
			if(penalties[[2]][r3[1],best_pick] > penalties[[2]][r3[1],r3[2]]){
				r3[,slev] <- best_pick
			}
		}
		else{
			if(penalties[[slev]][r3[slev-2],r3[slev-1],best_pick] > penalties[[slev]][r3[slev-2],r3[slev-1],r3[slev]]){
				r3[,slev] <- best_pick #???????????????????
			}	
		}
	}

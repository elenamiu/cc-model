...
zrandom <- matrix(0,nrow=10, ncol=nRounds)
#run 10 replicates
for (run in c(1:10)){
    print(paste("run ",run))
    z1 <- rep(0,nRounds)
	...
	m <- matrix(0, nrow=N, ncol=300)
	r <- matrix(0, nrow=n, ncol=300)
 	for (s in 1:ncol(r)){
		nreward <- rexp(n, rate=1)
		r[,s] <- round(2*(nreward^2))
	}
    #SIMULATION
    for (j in c(1:nRounds)){
        #replace pop with N naive individuals
        ...
        #assign list of effort budgets for each individual
        e <- rep(l,N)
		#pick models for copying

		models <- sample(c(1:N),N)
        # for every naive individual
        for (i in c(1:N)){
			if (j>1){
	            ##random copying
				#check if agent has enough effort budget to learn all
				model <- models[i]
				smax <- match(0,m[model,])-1#see how many levels model knows
				#check if the agent has enough effort budget to learn everything 
				if (smax <= floor(e[i]/cs)){
					m_naive[i,] <- m[model,]
					e[i] <- e[i] - smax * cs
				}else{
					#how many can it learn?
					slearn <- floor(e[i]/cs)
					m_naive[i,] <- m[model, 1:slearn]
					e[i] <- 0
				}
			}
            #INNOVATION  
            ...
            while(e[i] > 0){
				...
				if (!is.na(new)){
					...
					print("uh oh matrices not big enough")
					break
				}  
            }
        }
        ...
        m <- m_naive    
        #figure out average population complexity at each time round
        #   mean individual fitness at the pop level
        zpop <- rep(0,N)
		#for each individual
        for(i in c(1:N)){
            smax <- match(0,m[i,])-1
